...
class ApiService {
    ...
    get_data(location, start, end, supplier, tariff) {
        return fetch(`http://46.101.92.174/api/v1/optimize/?city=${location}&start=${start}&end=${end}&supplier=${supplier}&tariff=${tariff}`, {
            method: 'GET',
            mode: 'cors',
        })
    }
    ...
    deadhours(location, start, end, supplier, tariff, hours) {
        ...
        return fetch(`http://46.101.92.174/api/v1/deadhours/?city=${location}&start=${start}&end=${end}&supplier=${supplier}&tariff=${tariff}&hours=${hours.map((d,i) => {return d ? i : null}).filter((d) => d != null).join("&hours=")}`, {
            method: 'GET',
            mode: 'cors',
        })
    }
